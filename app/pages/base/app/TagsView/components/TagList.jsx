...
import { deleteTag, emptyTaglist, closeOtherTags } from "@actions/tagsView";
...
class TagList extends Component {
  ...
  handleClose = (tag) => {
    ...
    const path = tag.resKey;
    const currentPath = history.location.pathname;
    const length = taglist.length;
    // 如果关闭的是当前页，跳转到最后一个tag
    if (path === currentPath) {
      history.push(`/${taglist[length - 1].resKey}`);
    }
    // 如果关闭的是最后的tag ,且当前显示的也是最后的tag对应的页面，才做路由跳转
    if (
      path === taglist[length - 1].resKey &&
      currentPath === taglist[length - 1].resKey
    ) {
      // 因为cutTaglist在最后执行，所以跳转到上一个tags的对应的路由，应该-2
      if (length - 2 > 0) {
        history.push(`/${taglist[length - 2].resKey}`);
      } else if (length === 2) {
        history.push(`/${taglist[0].resKey}`);
      }
    }
      ...
      <>
        ...
        >
          <ul className="tags-wrap" ref={this.tagListContainer}>
            <li>
              <Tag onClick={this.handleClick.bind(null, 'index')}>
                首页
              </Tag>
            </li>
            {taglist && taglist.map((tag) => (
              <li key={tag.resKey}>
                <Tag
                  ...
                  closable={tag.resKey !== "/index"}
                  color={currentPath === tag.resKey ? "geekblue" : "gold"}
                  onClick={this.handleClick.bind(null, tag.resKey)}
