...
import { fetchPackRecord, fetchWarehousesAlls } from '@actions/pavo'
...
class packManage extends Component {
  ...
  getData = () => {
    this.props.fetchPackRecord({pageNumber:'0', pageSize: '10'}).then(res=>{
      this.setState({
  ...
  handleSearch = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.fetchPackRecord({...values, pageNumber:'0', pageSize: '10'}).then(res=>{
          this.setState({
            list: res.data.content,
            loading: false
          })
              ...
              <div className="page-header">
                <div className="layout-between">
                  <Form style={{ width: '100%' }} onSubmit={this.handleSearch}>
                    ...
                    <Row>
                      <Col span={6}><FormItem><Button size="small" htmlType="submit">查询</Button><Button size="small" type="primary" onClick={this.handleAdd}>打包</Button></FormItem></Col>
                    </Row>
                  </Form>
                </div>
              </div>
...
export default connect((state) => state.warehouseManage, { fetchPackRecord, fetchWarehousesAlls })(packManage)